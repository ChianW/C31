...
**步骤 4.1**: 确认 Plan 文件存在且为 active 状态

**步骤 4.2**: 加载 `skills/C31-lfg/SKILL.md`

**步骤 4.3**: 用户确认"开干"后，启动 9-Gate 全自动流水线：
- Gate 0: 加载 Plan
- Gate 1: Plan 验证（Nyquist 三问）
- Gate 2: 创建隔离 worktree
- Gate 3-4: 按 Wave 并行/串行执行（子代理分发，同 C31-work 机制）
- Gate 5: 自动 Review
- Gate 6: 自动修复（最多 3 轮）
- Gate 7: 原子提交 + 更新 STATE.md
- Gate 8: 残差移交（未完成项生成 ticket）
- Gate 9: 生成执行报告

**步骤 4.4: 用户验收**
- 向用户展示 lfg 执行报告摘要（完成的 units、review 结果、残差项）
- 用户检查产出：测试功能、review 代码、试用验证
- 验收结果：
  - **通过** → 进入步骤 4.5
  - **不通过** → 记录问题清单，回到修复流程（可调用 C31-work 或重新跑 LFG）

> **严禁未经用户验收自动 compound。** C31-compound 只有在用户明确说"记录一下"/"C一下"时才触发。

**步骤 4.5: 收尾（gsd-ship）**
- 用户确认验收通过后，加载 `skills/gsd-ship/SKILL.md`
- 执行收尾：生成阶段总结、更新 STATE.md、标记 shipped
- 可选：用户决定是否触发 C31-compound（知识归档）

> 注意: C31-lfg 是全自动执行，一旦启动中间不停车。如需 Wave 级暂停检查，改用 `C31-work` 手动模式。
...
- `skills/C31-lfg/SKILL.md` — 全自动执行（9-Gate 流水线）
- `skills/C31-work/SKILL.md` — 手动执行（如需 Wave 级暂停检查）
