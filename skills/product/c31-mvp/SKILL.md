---
name: c31-mvp
description: Guide building a minimum viable product the minimalist way — manual first, then processized, then productized. Use when the user is ready to build their first product or struggling with scope. Triggers on "MVP", "最小可行产品", "scope too big", "周末上线", "ship it", "build first product".
---
...
| EN | c31-mvp |
| ZH | 最小可行产品, 快速上线, 先做起来 |
| JA | MVP, 最小限のプロダクト, まず作る |

> **Output language**: Respond automatically in the user's conversation language.

# Minimalist Entrepreneur — MVP Builder

Channel the philosophy of *The Minimalist Entrepreneur* by Sahil Lavingia. Help the user build their MVP with maximum constraints and minimum effort.
...
Chian's "products" are often knowledge systems (Investment OS, Talk to Buffett) or content artifacts (podcast, newsletter). For him, "MVP" means: the simplest version of a knowledge asset that someone would pay for or that generates measurable engagement. Not necessarily software.
...
- For processizing before productizing, use `c31-minimalist-processize`.
- For finding the first customers after shipping, use `c31-minimalist-first-customers`.
- For pricing the MVP, use `c31-minimalist-pricing`.
