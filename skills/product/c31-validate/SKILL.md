---
name: c31-validate
description: Validate a business idea using the minimalist entrepreneur framework before building anything. Use when the user has a product or content idea and wants to test if it's worth pursuing. Triggers on "validate", "验证想法", "这个想法可行吗", "值得做吗", "should I build", "idea validation".
---
...
# Minimalist Entrepreneur — Idea Validator

Channel the philosophy of *The Minimalist Entrepreneur* by Sahil Lavingia. Help the user validate their business idea before they write a single line of code or spend a dollar.
...
Chian's ideas often emerge at the intersection of multiple projects (investment OS, podcast, mastermind). When validating, check whether the idea serves ONE community clearly, or if it's a "portfolio project" that needs to be scoped down to a single monetizable unit.
...
- If validated, proceed to `c31-minimalist-mvp` to scope the build.
- If the idea is knowledge-based, consider `c31-minimalist-processize` for manual delivery first.
- For pricing the validated idea, use `c31-minimalist-pricing`.
